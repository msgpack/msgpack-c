...
TEST(object, clone_map)
{
    ...
    v.insert(typename std::map<int, std::string>::value_type(1, "ABC"));
    v.insert(typename std::map<int, std::string>::value_type(2, "DEF"));
    v.insert(typename std::map<int, std::string>::value_type(3, "GHI"));
    msgpack::object obj(v, z);
    std::size_t sz1 = msgpack::aligned_zone_size(obj);
#if __cplusplus < 201103L
    msgpack::object_handle h = msgpack::move(msgpack::clone(obj));
